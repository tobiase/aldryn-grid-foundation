...
class GridColumnFoundation(CMSPlugin):
    size_small = models.IntegerField(_('size (mobile)'), choices=ALDRYN_GRID_FOUNDATION_CHOICES, default=GRID_CONFIG['COLUMNS'])
    size_large = models.IntegerField(_('size (desktop)'), choices=[[None, '']] + ALDRYN_GRID_FOUNDATION_CHOICES, default=8, blank=True, null=True)
    custom_classes = models.CharField(_('custom classes'), max_length=200, blank=True)

    def __unicode__(self):
        result = ['small-%d' % self.size_small]
        if self.size_large:
